            ...
            <button
              onClick={() => {
                ...
                if (!isNaN(num) && num > 0) {
                  ...
                  onClose();
                  toast.success(
                    `Budget updated: ${category.name} → ${fmt(num)}/mo`,
                  );
                }
              }}
...
export function Budget() {
  ...
  const getSpentForCategory = (catName: string) => {
    return (
      currentMonthExpensesByCategory.find((c) => c.category === catName)
        ?.amount || 0
    );
  };

  const overBudgetCount = budgets.filter(
    (b) => getSpentForCategory(b.name) > b.limit,
  ).length;
    ...
    <div className="p-6 lg:p-10 max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        {overBudgetCount > 0 && (
          <div className="flex items-center gap-3 px-5 py-3 rounded-[1.25rem] border border-destructive/20 bg-destructive/10 shadow-sm animate-in slide-in-from-right-4 duration-500">
            ...
            <span className="text-[14px] font-bold text-destructive">
              {overBudgetCount}{" "}
              {overBudgetCount === 1 ? "category" : "categories"} over budget
            </span>
          </div>
        )}
      </div>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-10 relative z-10">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-6 w-full">
            {[
              ...
              <div key={label} className="text-center sm:text-left">
                ...
                <p
                  className={`text-3xl font-bold tracking-tight leading-none mb-1.5 ${color}`}
                >
                  {value}
                </p>
          ...
          <div className="w-full lg:w-64 shrink-0 bg-background/60 backdrop-blur-md rounded-2xl p-5 border border-border shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`text-[13px] font-bold ${overallPct > 100 ? "text-destructive" : overallPct > 80 ? "text-amber-500" : "text-emerald-500"}`}
              >
                {overallPct}%
              </span>
            </div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[15px] font-bold text-card-foreground group-hover:text-primary transition-colors">
                        {b.name}
                      </p>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span
                      className={`text-[16px] font-bold ${isOver ? "text-destructive" : "text-card-foreground"}`}
                    >
                      {fmt(spent)}
                    </span>
                  ...
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border shadow-sm ${statusBg} ${statusColor}`}
                  >
                    {statusText}
                  </span>
                </div>
          ...
          {goals.map((goal) => {
            ...
            return (
              ...
              >
                ...
                {/* Info */}
                <div>
                  <p className="text-[15px] font-bold text-card-foreground mb-1">
                    {goal.name}
                  </p>
                  <p className="text-[13px] font-medium text-muted-foreground">
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() =>
              toast.info("Manage recurring transactions coming soon!")
            }
            ...
            >
              ...
              >
                <TrendingUp
                  ...
                  style={{
                    color: item.color,
                    transform: item.type === "expense" ? "scaleY(-1)" : "none",
                  }}
                />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[14.5px] font-bold text-card-foreground truncate">
                  {item.name}
                </p>
                <p className="text-[12px] font-medium text-muted-foreground">
                  {item.freq}
              ...
              <span
                className={`text-[15px] font-bold shrink-0 ${item.type === "income" ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}
              >
                {item.type === "income" ? "+" : "-"}
                {fmt(item.amount)}
              </span>
