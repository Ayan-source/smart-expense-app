  ...
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl shadow-xl border border-border bg-popover px-4 py-3 space-y-1.5 animate-in zoom-in-95 duration-200">
        <p className="text-[12px] font-bold text-muted-foreground uppercase tracking-wider mb-2">{label} 2026</p>
        {payload.map((p: any) => (
          <div key={p.name} className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full inline-block shadow-sm" style={{ background: p.color }} />
            <span className="text-[13px] font-medium text-muted-foreground capitalize">{p.name}:</span>
            <span className="text-[14px] font-bold text-foreground">{fmt(p.value)}</span>
          </div>
        ))}
      </div>
    );
  ...
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl shadow-xl border border-border bg-popover px-4 py-3 animate-in zoom-in-95 duration-200">
        <p className="text-[12px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{label} 2026</p>
...
export function Reports() {
  ...
  const avgSavingsRate = totalAllIncome > 0 ? Math.round((netSavings / totalAllIncome) * 100) : 0;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {[
          ...
          },
          {
            ...
            valueColor: avgSavingsRate >= 20 ? 'text-teal-600 dark:text-teal-400' : avgSavingsRate >= 10 ? 'text-amber-500 dark:text-amber-400' : 'text-destructive',
          },
          ...
          >
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 border shadow-inner group-hover:scale-105 transition-transform ${iconBg}`}>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-6 flex items-center gap-5 hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <p className="text-[13px] font-medium text-emerald-600 dark:text-emerald-500/80">
              Income {fmt(bestMonth?.income || 0)} — Expenses {fmt(bestMonth?.expenses || 0)}
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-6 flex items-center gap-5 hover:shadow-md transition-shadow">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 bg-rose-500/10 border border-rose-500/20 shadow-sm">
            <ArrowDownRight size={24} className="text-rose-600 dark:text-rose-400" />
          </div>
          <div>
      ...
      {activeTab === 'overview' ? (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                <h3 className="text-[16px] font-bold text-card-foreground">Monthly Income vs Expenses</h3>
                <p className="text-[13px] text-muted-foreground font-medium mt-1">Comparison across all months · Jan – May 2026</p>
              </div>
              <div className="flex items-center gap-5 bg-muted/50 px-3 py-1.5 rounded-lg border border-border">
                <span className="flex items-center gap-2 text-[12px] font-bold text-foreground">
                  <span className="w-3 h-3 rounded-md shrink-0 shadow-sm" style={{ background: chartColors.incomeBar }} />
                  Income
                </span>
                <span className="flex items-center gap-2 text-[12px] font-bold text-foreground">
                  <span className="w-3 h-3 rounded-md shrink-0 shadow-sm" style={{ background: chartColors.expenseBar }} />
                  Expenses
                </span>
              </div>
            </div>
            <div className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlySummary} margin={{ top: 10, right: 10, left: -10, bottom: 0 }} barGap={6}>
                  <CartesianGrid strokeDasharray="4 4" stroke={chartColors.gridLine} vertical={false} />
                  <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: chartColors.text, fontWeight: 500 }} dy={10} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: chartColors.text, fontWeight: 500 }} tickFormatter={fmtShort} dx={-10} />
                  <Tooltip content={<CustomTooltip />} cursor={{ fill: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)' }} />
                  <Bar dataKey="income" name="income" fill={chartColors.incomeBar} radius={[6, 6, 0, 0]} maxBarSize={48} />
                  <Bar dataKey="expenses" name="expenses" fill={chartColors.expenseBar} radius={[6, 6, 0, 0]} maxBarSize={48} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-3 rounded-2xl border border-border bg-card p-6 shadow-sm">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <h3 className="text-[16px] font-bold text-card-foreground">Cumulative Balance Trend</h3>
                  <p className="text-[13px] text-muted-foreground font-medium mt-1">Running net savings growth over time</p>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-bold text-[13px]">
                  ...
                  {fmt(monthlySummary[monthlySummary.length - 1]?.cumulativeBalance || 0)}
                </div>
              </div>
              <div className="h-[240px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={monthlySummary} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                    <defs>
                      <linearGradient id="balanceGradReport" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={chartColors.balanceLine} stopOpacity={0.25} />
                        <stop offset="100%" stopColor={chartColors.balanceLine} stopOpacity={0.02} />
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="4 4" stroke={chartColors.gridLine} vertical={false} />
                    <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: chartColors.text, fontWeight: 500 }} dy={10} />
                    ...
                    <Area
                      ...
                      dot={{ fill: chartColors.balanceLine, r: 4, strokeWidth: 2, stroke: isDark ? '#0f172a' : '#fff' }}
                      activeDot={{ r: 6, fill: chartColors.balanceLine, strokeWidth: 0, shadowBlur: 10, shadowColor: chartColors.balanceLine }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="lg:col-span-2 rounded-2xl border border-border bg-card p-6 shadow-sm flex flex-col">
              <div className="mb-6">
                <h3 className="text-[16px] font-bold text-card-foreground">Spending by Category</h3>
                <p className="text-[13px] text-muted-foreground font-medium mt-1">All-time expense distribution</p>
              </div>
                ...
                <div className="space-y-3 mt-4">
                  {allTimeExpenseByCategory.slice(0, 5).map(item => (
                    <div key={item.category} className="flex items-center justify-between group">
                      <div className="flex items-center gap-3">
                        <span className="w-3 h-3 rounded-full shrink-0 shadow-sm transition-transform group-hover:scale-125" style={{ background: item.color }} />
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-[12px] font-bold text-muted-foreground">{Math.round(item.percentage)}%</span>
          ...
          <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden flex flex-col">
            <div className="px-6 py-5 border-b border-border bg-muted/20">
              <h3 className="text-[16px] font-bold text-card-foreground">Monthly Performance Summary</h3>
              <p className="text-[13px] font-medium text-muted-foreground mt-1">Detailed breakdown by month</p>
            </div>
            <div className="overflow-x-auto flex-1">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-muted/40">
                    {['Month', 'Income', 'Expenses', 'Net Balance', 'Savings Rate', 'Status'].map(h => (
                      <th
                        key={h}
                ...
                <tbody className="divide-y divide-border/60">
                  {[...monthlySummary].reverse().map((m, i) => {
                    ...
                    return (
                      <tr key={m.month} className="hover:bg-muted/30 transition-colors group">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <span className="text-[14px] font-bold text-card-foreground group-hover:text-primary transition-colors">{m.month} 2026</span>
                            {isBest && (
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 shadow-sm">BEST</span>
                            )}
                            {isWorst && !isBest && (
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 shadow-sm">HIGH SPEND</span>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`text-[15px] font-bold ${m.balance >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                            ...
                            <div className="w-20 h-2 rounded-full bg-muted overflow-hidden">
                              <div
                                ...
                                style={{
                                  ...
                                  background: rate >= 20 ? '#10b981' : rate >= 10 ? '#f59e0b' : '#ef4444',
                                }}
                              />
                            </div>
                            <span className={`text-[13px] font-bold ${rate >= 20 ? 'text-emerald-600 dark:text-emerald-400' : rate >= 10 ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'}`}>
                              {rate}%
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          >
                            {rate >= 20 ? 'On Track' : rate >= 10 ? 'Moderate' : 'Review'}
                          </span>
                        </td>
                      </tr>
                    );
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative z-10">
              <div>
                ...
                <p className="text-[14px] font-medium text-muted-foreground leading-relaxed max-w-md">
                  Export your complete financial history as a PDF summary or CSV spreadsheet for personal records or accounting.
                </p>
              </div>
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="px-6 py-5 border-b border-border bg-muted/20 flex items-center justify-between">
            <div>
              <h3 className="text-[16px] font-bold text-card-foreground">All Transactions</h3>
              <p className="text-[13px] font-medium text-muted-foreground mt-1">{sortedTransactions.length} total entries</p>
            </div>
            <button
              onClick={() => handleExport('csv')}
              ...
              <tbody className="divide-y divide-border/60">
                {sortedTransactions.map(tx => (
                  <tr key={tx.id} className="hover:bg-muted/30 transition-colors group">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <p className="text-[14.5px] font-bold text-card-foreground group-hover:text-primary transition-colors">{tx.title}</p>
                        {tx.note && (
                          <p className="text-[12px] font-medium text-muted-foreground mt-0.5">{tx.note}</p>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`capitalize text-[13px] font-bold ${tx.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                          {tx.type}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`text-[15px] font-bold ${tx.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
