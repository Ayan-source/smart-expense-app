  ...
  return (
    <div className="rounded-[1.25rem] p-6 border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 flex flex-col gap-5 group">
      <div className="flex items-start justify-between">
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-inner group-hover:scale-105 transition-transform ${iconBgClass}`}>
      ...
      <div>
        ...
        <p className={`text-3xl font-bold tracking-tight leading-none ${valueColorClass}`}>
          {value}
        </p>
      </div>
    </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl shadow-xl border border-border bg-popover px-4 py-3">
        <p className="text-[12px] text-muted-foreground font-medium mb-1">{label} 2026</p>
  ...
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl shadow-xl border border-border bg-popover px-4 py-3 space-y-1.5">
        <p className="text-[12px] text-muted-foreground font-medium mb-2">{label} 2026</p>
        {payload.map((p: any) => (
          <div key={p.name} className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full inline-block shadow-sm" style={{ background: p.color }} />
            <span className="text-[13px] text-muted-foreground capitalize font-medium">{p.name}:</span>
    ...
    <div className="p-6 lg:p-10 space-y-8 max-w-[1500px] mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight mb-1">
            Good morning, Alex <span className="animate-wave inline-block origin-[70%_70%]">👋</span>
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <SummaryCard
          ...
          valueColorClass={savingsRate >= 20 ? 'text-teal-600 dark:text-teal-400' : savingsRate >= 10 ? 'text-amber-500 dark:text-amber-400' : 'text-rose-500 dark:text-rose-400'}
        />
      </div>
      ...
      <div className="grid lg:grid-cols-5 gap-5">
        ...
        <div className="lg:col-span-3 rounded-2xl border border-border bg-card p-6 shadow-sm flex flex-col">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-[13px] text-muted-foreground font-medium mt-1">Running total across Jan – May 2026</p>
            </div>
        ...
        <div className="lg:col-span-2 rounded-2xl border border-border bg-card p-6 shadow-sm flex flex-col">
          <div className="mb-6">
            <h3 className="text-[16px] font-bold text-card-foreground">Spending Breakdown</h3>
            <p className="text-[13px] text-muted-foreground font-medium mt-1">May 2026 by category</p>
          </div>
          
          {currentMonthExpensesByCategory.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
              <PieChart size={48} className="text-muted-foreground/30 mb-3" />
              <p className="text-[14px] font-medium text-muted-foreground">No expense data yet</p>
            </div>
          ) : (
      ...
      <div className="grid lg:grid-cols-5 gap-5">
        ...
        <div className="lg:col-span-3 rounded-2xl border border-border bg-card p-6 shadow-sm flex flex-col">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h3 className="text-[16px] font-bold text-card-foreground">Income vs Expenses</h3>
              <p className="text-[13px] text-muted-foreground font-medium mt-1">Monthly comparison, Jan – May 2026</p>
            </div>
          ...
          <div className="flex-1 min-h-[240px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlySummary} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barGap={6}>
                <CartesianGrid strokeDasharray="4 4" stroke={chartColors.gridLine} vertical={false} />
                <XAxis
                  dataKey="month"
                  axisLine={false}
                ...
                <YAxis
                  ...
                  dx={-10}
                />
                <Tooltip content={<CustomBarTooltip />} cursor={{ fill: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)' }} />
                <Bar dataKey="income" name="income" fill={chartColors.incomeBar} radius={[6, 6, 0, 0]} maxBarSize={40} />
                <Bar dataKey="expenses" name="expenses" fill={chartColors.expenseBar} radius={[6, 6, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 rounded-2xl border border-border bg-card p-6 shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-[16px] font-bold text-card-foreground">Recent Transactions</h3>
            <button
          ...
          <div className="space-y-1 flex-1 overflow-y-auto pr-1 -mr-2">
            {recentTransactions.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full py-10 text-center">
                ...
                <p className="text-[14px] font-bold text-muted-foreground">No transactions yet</p>
                <p className="text-[13px] text-muted-foreground/70 font-medium mt-1">Add income or expenses to get started</p>
              </div>
            ) : (
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-[16px] font-bold text-card-foreground">Savings Goals</h3>
            <p className="text-[13px] text-muted-foreground font-medium mt-1">Progress toward your financial targets</p>
          </div>
          <button
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { name: 'Emergency Fund', target: 10000, saved: 6500, color: '#6366f1', icon: Wallet },
            { name: 'Europe Vacation', target: 3000, saved: 1200, color: '#0d9488', icon: Plane },
            { name: 'New Laptop', target: 2200, saved: 880, color: '#f59e0b', icon: Laptop },
            { name: 'Education Fund', target: 5000, saved: 2750, color: '#10b981', icon: GraduationCap },
          ].map(goal => {
            ...
            return (
              <div key={goal.name} className="space-y-3 group">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 shadow-sm" style={{ background: `${goal.color}15` }}>
                    <Icon size={16} style={{ color: goal.color }} />
                  </div>
                  <p className="text-[14px] font-bold text-card-foreground">{goal.name}</p>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[12px] font-medium text-muted-foreground">{fmt(goal.saved)} / {fmt(goal.target)}</span>
                </div>
              </div>
            );
          })}
      ...
      <div className="rounded-2xl border border-indigo-500/20 bg-gradient-to-br from-indigo-500/10 via-primary/5 to-teal-500/10 p-6 shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 relative z-10">
          ...
          >
            View Full Report <ChevronRight size={16} className="text-muted-foreground group-hover:text-foreground transition-colors" />
          </button>
        </div>
      </div>
    </div>
